...
const sortByChildren = (charArray) => {
  let ordered = charArray.sort((char1, char2) => {
    if (char1.children.length === char2.children.length) {
      return charArray.sort((char1, char2) =>
        char1.house.localeCompare(char2.house)
      );
    } else {
      return char1.children.length - char2.children.length;
    }
  });
  return ordered;
};
