...
const updateNumbers = (obj) =>
  Object.entries(obj).map(
    (contactInfo) => `${contactInfo[0]}: ${contactInfo[1]}`
  );
...
const getHouses = (arr) => {
  return arr.map((entry) => entry.house);
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach((characterObj) => {
    if (
      Object.values(characterObj).includes(character) &&
      Object.keys(characterObj).includes("children")
    ) {
      hasChildren = true;
    }
  });
