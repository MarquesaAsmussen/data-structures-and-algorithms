...
const longestString = (arr) => {
  let answer = "";
  arr.forEach((str) => (str.length > answer.length ? (answer = str) : answer));
...
const standardizePhoneNumbers = (arr) => {
  let standardizedNumArr = [];
  arr.forEach((str) => {
    standardizedNumArr.push(str.replace(/\D/g, ""));
  });
  return standardizedNumArr;
};
...
const onlyOddChars = (str) => {
  let spreadArr = [...str];
  let oddArr = spreadArr.filter((char, i) => i % 2 !== 0).join("");
  return oddArr;
};
...
const allHappy = (arr) => {
  ...
  // first, check each string for ':)'
  arr.forEach((str) => (str.includes(":)") ? booleanArr++ : booleanArr));

  // then, return boolean
  return booleanArr === arr.length ? true : false;
};
