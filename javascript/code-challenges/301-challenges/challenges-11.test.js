...
function transformToLis(obj) {
  ...
  let listArr = Object.entries(obj);
  return listArr.map((arr) => `<li>${arr[0]}: ${arr[1]}</li>`);
}
...
const count = (target, input) => {
  ...
  input.map((arr) =>
    arr.map((integer) => {
      if (integer === target) {
        accumulatingArr.push(1);
      }
    })
  );
  let total = accumulatingArr.reduce((a, b) => a + b, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach((arr) => {
    let filteredArr = arr.filter(
      (value) => Number.isInteger(value) === true && value % 5 === 0
    );
    allFilteredArr.push(filteredArr.map((integer) => Math.pow(2, integer)));
  });

  return allFilteredArr;
};
...
// find least value "x" in an array of objects
//

let findShortest = (data) => {
  return data.reduce((R2, comparedChar) =>
    parseInt(R2.height) < parseInt(comparedChar.height) ? R2 : comparedChar
  ).name;
};
